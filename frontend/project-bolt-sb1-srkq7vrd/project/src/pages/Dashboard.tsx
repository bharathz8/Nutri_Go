...
const Dashboard: React.FC = () => {
  ...
  // Simplified translation state - only for main content
  ...
  useEffect(() => {
    if (user) {
      fetchDailyIntake();
      setCurrentLanguage(user.preferred_language || 'english');
    }
  }, [user, selectedDate]);
  ...
  const handleMainTranslation = (translatedText: string, targetLanguage: string) => {
    // Split the translated text to extract welcome and subtitle
    const parts = translatedText.split('!');
    if (parts.length >= 2) {
      setTranslatedWelcome(parts[0] + '!');
      setTranslatedSubtitle(parts[1].trim());
    } else {
      setTranslatedWelcome(translatedText);
      setTranslatedSubtitle('Track your daily nutrition and stay on top of your health goals.');
    }
    setCurrentLanguage(targetLanguage);
  };

  const getWelcomeText = () => {
    return translatedWelcome || `Welcome back, ${user?.user_id}!`;
  };

  const getSubtitleText = () => {
    return translatedSubtitle || 'Track your daily nutrition and stay on top of your health goals.';
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-7xl mx-auto">
        {/* Header with Translation - SINGLE TRANSLATION TOGGLE */}
        <div className="mb-8 flex items-start justify-between">
          ...
          <div className="ml-4">
            <TranslationToggle
              text={`Welcome back, ${user.user_id}! Track your daily nutrition and stay on top of your health goals.`}
              currentLanguage={currentLanguage}
              onTranslatedText={handleMainTranslation}
            />
          </div>
        </div>
